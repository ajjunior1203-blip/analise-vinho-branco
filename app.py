...
# 3. TEXTO REVISADO (BLINDADO CONTRA MARCAÇÕES)
st.markdown("---")
st.header("Sobre o Estudo")

texto_estudo = f"""
Este projeto apresenta uma interface interativa construída por meio da mineração de dados aplicados a testes laboratoriais e sensoriais do Vinho Verde Branco, uma variante única produzida na região demarcada do Norte de Portugal. O principal objetivo consiste em modelar as preferências humanas e estimar a qualidade da bebida com base puramente em seus ensaios físico-químicas.

### Como o estudo foi realizado?
* **As Amostras:** Foram coletadas e analisadas **{len(df):,}** amostras reais de vinho branco pertencentes à base de dados oficial.
* **Avaliação Sensorial:** Cada amostra passou por testes às cegas aplicados por especialistas do setor. A nota de qualidade final representa a mediana de pelo menos três avaliações independentes realizadas por esses sommeliers, mapeada em uma escala que vai de 0 (muito ruim) a 10 (excelente).
* **Modelagem Estatística:** O comportamento dessas avaliações foi estruturado sob uma perspectiva matemática de regressão. Essa técnica utiliza as 11 propriedades físico-químicas de entrada para treinar o algoritmo inteligente que você encontra ao final do painel.

### O que significam as variáveis analisadas?
Para tornar a compreensão simples e direta, os 11 atributos químicos avaliados no estudo foram divididos em três dimensões principais:
1. **Estrutura e Acidez:** Reúne a Acidez Fixa, a Acidez Volátil, o Ácido Cítrico e o índice de pH. Esses elementos controlam o equilíbrio de sabores e garantem a estabilidade microbiológica da bebida.
2. **Corpo e Doçura:** Bloco composto pelo Açúcar Residual (remanescente do processo de fermentação), pela Densidade do líquido e pelo teor de Álcool. Juntos, eles ditam a percepção de volume, peso e consistência no paladar.
3. **Estabilização e Conservação:** Contempla os Cloretos, os Sulfatos e as divisões de Dióxido de Enxofre Livre e Total. Esses componentes desempenham um papel vital na proteção antioxidante, evitando que o vinho estrague ou oxide com o tempo.

---
### Créditos e Licenciamento Oficial
Este ecossistema foi disponibilizado publicamente pelo **UC Irvine Machine Learning Repository**. Os dados científicos originais foram gerados e publicados por **Paulo Cortez** (Universidade do Minho), **António Cerdeira**, **Fernando Almeida**, **Telmo Matos** e **José Reis** em 2009.

* **Citação Acadêmica Recomendada:** Cortez et al., 2009. Modeling wine preferences by data mining from physicochemical properties. In Decision Support Systems, Elsevier, 47(4):547-553.
* **Licença de Uso:** O banco de dados está licenciado sob a licença global **Creative Commons Atribuição 4.0 Internacional (CC BY 4.0)**, permitindo o compartilhamento, adaptação e uso livre sob atribuição de créditos.
"""

# Filtro definitivo para limpar qualquer marcação de [cite] que persista por cache ou dados externos
texto_limpo = texto_estudo.replace(',', '.')
for i in range(100):
    texto_limpo = texto_limpo.replace(f"", "").replace(f"", "")
    for j in range(100):
        texto_limpo = texto_limpo.replace(f"", "").replace(f"", "")
